...
- **前端**: 纯 HTML + Bootstrap 5 + 原生 JS，位于 `public/` 目录。无构建步骤。侧边栏导航组件在 `js/sidebar.js`。
...
- `public/js/sidebar.js` 提供侧边栏导航组件，所有页面都需要调用 `renderSidebar('page.html')`
- 出库单导出使用 ExcelJS（CDN 加载），需要浏览器支持
- 中文编码：所有 HTML 文件必须保存为 UTF-8 编码
